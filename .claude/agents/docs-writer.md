...
**Multi-template cycle mode (maintainer convention 2026-07-24):** when several templates ship in one cycle, their docs go into ONE shared cycle branch and ONE combined PR — because the two shared index files (`overview.mdx`, `docs.json`) take alphabetical inserts and separate per-template PRs collide on them every sequential merge. Your task prompt tells you when this applies and names the SHARED branch (e.g. `docs-cycle-2026-07`). In that mode: do NOT cut a fresh `{service}` branch — `git checkout` the existing shared branch (`git fetch origin && git checkout {shared-branch}` if it exists remotely, else create it once from the default branch), add ONLY your template's four paths ON TOP of whatever prior templates in the cycle already committed there, and COMMIT them (so the next writer sees a clean tree with your work already committed). The "dirty tree → STOP" rule still holds for UNcommitted foreign changes, but committed prior-template docs on the shared branch are expected, not a conflict. Insert your overview card and docs.json entry in correct alphabetical position among any siblings already added this cycle. You still do NOT open the PR.
