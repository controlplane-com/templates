...
Alongside the architecture spec, write `briefing-{service}.md` (or the path the task prompt names) — a CONCISE maintainer-facing summary the maintainer reads to understand and later support the template. Format is fixed: **bullets and small tables only, no prose paragraphs, ~40-60 lines max**. Sections: **What it is** (1-2 lines incl. license), **Common use cases** (3-4 bullets), **Architecture on cpln** (a resource|purpose table + 1-2 bullets on shape), **Key knobs** (one compact line/table), **Troubleshooting / considerations** (5-8 bullets — the traps, invariants, and things-to-know-when-a-user-calls; this is the most valuable section). Summarize proposal + spec; do not duplicate the spec's detail. **Plain-language rule (maintainer, 2026-07-22): never use an uncommon acronym or specialist term without an inline gloss** — e.g. "AGPL (a strong open-source license: anyone offering a modified version as a service must share their changes)", "quorum (majority of replicas must agree)". Keep the technical wording, add the five-word explanation after it; the maintainer should never need to look a term up. The orchestrator keeps this file current when build/test reality diverges from the spec.

## Availability assessment (required in every spec — maintainer directive 2026-07-23)

Near-zero downtime is core to Control Plane; templates follow suit whenever possible. Every spec MUST contain an explicit horizontal-scaling assessment:
1. **Is multi-instance in the free/OSS edition?** Verify honestly (n8n multi-main = enterprise-gated → excluded with the reason stated; temporal/mimir-class = OSS-supported → offer it).
2. **Is it platform-feasible?** Peer discovery + cross-replica traffic on cpln. Proven precedents: mimir (memberlist VIP-join), temporal (database membership table). If unproven for this service, MANDATE a build-stage spike before the values surface freezes.
3. **When yes to both: ship a `replicas`-style knob** — default = the proven single-replica shape (byte-identical render), ≥N forms the cluster — and the Testability Map MUST include the availability rows: rolling-restart continuity (measured counts, not adjectives) and replica-down behavior.
4. The maintainer briefing states the availability posture either way, plainly.
