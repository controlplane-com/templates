...
- **Fourteen templates now pick up the etcd storage fix** — `cdc-pipeline` 1.0.2, `chatwoot` 1.0.1, `glitchtip` 1.0.1, `grafana` 1.2.1, `keycloak` 1.0.1, `listmonk` 1.0.1, `metabase` 1.0.1, `n8n` 1.0.1, `nocodb` 1.0.1, `polaris` 1.0.1, `temporal` 1.0.1, `twenty` 1.0.1, `umami` 1.0.1 and `unleash` 1.0.1 all bundle highly-available PostgreSQL, whose coordination store could fill its storage and force the database read-only after a couple of months. **Upgrade if you enabled the highly-available database option** — and note `cdc-pipeline` always bundles it, so every install of that template is affected
