...
- **Do not claim a CRS release has no Caddy build without paginating the tag list.** The registry returns 100 tags per page; a single unpaginated query showed caddy builds only up to 4.25 and that false claim reached the shipped README, the docs page and a customer. `?n=1000` returns 146 tags, 46 of them caddy, including four 4.28 variants. 1.2.1 pins `4.28-caddy-alpine-202608260808`.
- **Large-body disk buffering is real, and the `mkdir` is the load-bearing part.** Coraza spills a body over its in-memory limit to `/tmp/coraza/body<random>`. The 1.1.x `diskBodyInspection` knob did two things: `mkdir -p /tmp/coraza` (which WORKED and is what fixed a customer's large-body 500s) and setting `SecRequestBodyNoFilesLimit` (genuinely inert — swept 1 KB to 1 MB on both settings with a valid control, identical at every size). The knob was removed in 1.2.1 on the strength of the inert half alone, which threw out the working half. **Measured 2026-08-27 on the 4.7.0 image:** `/tmp/coraza` absent, a 600 KB POST returned 500; running 1.2.2's hook created the directory and the identical request returned 405. The 4.28 image creates the directory itself, so the mkdir is a no-op there — it is unconditional in 1.2.2 because `image` is user-settable and older builds still need it.
- **Lesson:** the knob's name described neither of its effects. When a knob does two things, test both before removing it — and test on the image the user actually reported against, not only the new pin.
