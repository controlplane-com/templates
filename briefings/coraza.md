...
- **Large-body disk buffering is real, and the `mkdir` is the load-bearing part.** Coraza spills a body over its in-memory limit to `/tmp/coraza/body<random>`. The 1.1.x `diskBodyInspection` knob did two things: `mkdir -p /tmp/coraza` (which WORKED and is what fixed a customer's large-body 500s) and setting `SecRequestBodyNoFilesLimit` (genuinely inert — swept 1 KB to 1 MB on both settings with a control shape that discriminates, identical at every size). The knob was briefly removed on the strength of the inert half alone, which threw out the working half. **Measured 2026-08-27 on the 4.7.0 image:** `/tmp/coraza` absent, a 600 KB POST returned 500; creating the directory took the identical request to 405. The 4.28 image creates it itself, so the mkdir is a no-op there — it is unconditional because `image` is user-settable and older builds still need it.
