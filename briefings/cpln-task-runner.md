...
- **`/v1/enqueue` is UNAUTHENTICATED and stays that way** — there is no per-client key in the binary (the only `*_CLIENT_KEY` strings belong to the OTEL exporter). It does reject an unregistered `client_id` (`client not found`), so on a public install the client ID is the whole access control. The README says so and recommends random client IDs.
