...
| Resource | Purpose |
|---|---|
| `gvc` (only when `createGvc: true`) | Multi-location GVC pinned to `global.gvc.locations` via `staticPlacement` |
...
- **This chart is `createsGvc: true`.** Pointing it at a GVC that already exists makes Helm adopt it, and `helm uninstall` will then **delete** that GVC and everything in it (this destroyed `test-gvc` on 2026-08-07 despite a `resource-policy: keep` annotation). Use `createGvc: false` for any pre-existing GVC — that is also the safe form for testing.
