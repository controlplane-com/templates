...
| `postgres.credentials.{username,password,database}` | `username` / `password` / `test` | **2.4.0** — was `postgres.config.*`; bundled plumbing, still plain values |
...
- **The database defaults are weak working credentials — `username` / `password` / `test`.** They violate the current placeholder convention (`change-me-fusionauth-db`), but they were NOT changed in 2.4.0: an installed 2.3.x that never overrode the password has a data directory initialized with `password`, and a new default would break that upgrade. Fixing it needs a deliberate migration note, so it is a tracked follow-up, not a drive-by.
