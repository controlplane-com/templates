...
- **"Why was Grafana 401/503 right after I followed the README?"** The first `helm upgrade` after an install re-applies the bundled subchart resources even when their render is byte-identical — `test-grafana-postgres` and its volumeset reported `Updated` with no database value changed — so the app DB restarts and logins 401 / `/api/health` 503s for **~2 minutes**. It hit the `applyPassword: false` test row and reads exactly like the toggle locked the user out; it is not (`admin:admin` 401s too, so nothing was reset). Same behavior bounced nocodb's bundled Redis; recorded as a platform gotcha in `CLAUDE.md`, **not** something to fix in the chart. The README's `applyPassword` note warns about it.
