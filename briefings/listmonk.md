...
| `{release}-listmonk-admin` secret (dictionary) | bootstrap Super Admin username/password |
| identity + policy | `reveal` on exactly the admin secret + the DB credentials secret (mode-aware: `-pg-config` or `-postgres-config`) |
...
| `admin.username` / `admin.password` | `admin` / `change-me-listmonk-admin` | **change before install**; min 3 / 8 chars (render-validated); first-install only |
...
- **Admin password value changed but login unchanged** → expected: `LISTMONK_ADMIN_*` is read only during first install; accounts live in the database afterwards. Change passwords in the UI (Admin → Users). Restart is proven idempotent (`skipping install as database appears to be already setup` — no re-seed, no superadmin re-create).
