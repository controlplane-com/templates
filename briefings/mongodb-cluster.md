...
- Optional HAProxy in front so clients hit one endpoint instead of tracking the primary, and optional logical (mongodump) or physical (PBM) backups.
...
| workload `{release}-mongo-backup` / `-physical-backup` (cron, optional) | Logical dump or PBM physical backup trigger |
