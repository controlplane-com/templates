...
- **PgBouncer in `transaction` mode loses session GUCs** — `SET hnsw.ef_search` silently reverts between statements, which shows up as *worse results*, never an error. Use `SET LOCAL` in a transaction, or `poolMode: session`.
