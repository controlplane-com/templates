...
- **Bundled Postgres backups are not available.** Plane supports PostgreSQL 15.7/16.x while the `postgres` template's backup cron requires 17+. The README ships a `pg_dump`/`pg_restore` procedure instead. **Be precise about what was proven:** the round-trip was exercised **locally, in a `postgres:16` container** holding a Plane-migrated database (dump → drop a table → restore → row back, 164 migrations intact). The **transport was NOT** — binary `pg_dump` stdout streaming out of `cpln workload exec`, and a local file forwarded into its stdin, is undocumented anywhere in this catalog. That is spec test row **T22**, and until it passes the README says so on the page. Do not point users at the subchart's backup block.
