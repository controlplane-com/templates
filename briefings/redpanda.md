...
- **The volumeset now ships snapshots** (`createFinalSnapshot` / `retentionDuration: 7d` / `schedule: 0 0 * * *`), matching kafka's surface. Added in 1.1.0 after the knobs audit flagged topic data as the only durable state with nothing backing it up. Declarative and platform-native, so it rode the existing test round rather than needing its own.
...
## Status
- **Deploy-tested at 1.1.0: 9 PASS, 0 FAIL.** Both high-risk rows were settled with in-image evidence — Console genuinely starts under the new startup script (`/bin/sh -> /bin/busybox`, `/app/console`, `exec` makes it PID 1, connected to all three brokers), and a hostile password (`Rp''Adm:#{}7x`) round-tripped correctly into both configs. Console closed by default confirmed (canonical 403, port-forward served the UI, re-opening flipped to 200 in ~15 s).
