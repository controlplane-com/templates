...
- **The broker Admin API (9644) is unauthenticated** and open to the whole GVC under the default `same-gvc` firewall. The readiness probe, the `postStart` hook and Console all rely on that. Turning on `http_basic` there would need probe/hook credentials too — a real follow-up, not a quiet edit.
