...
- **Temporal's TWO databases needed no extra secret, and that is worth knowing before someone "fixes" it.** The three-key secret covers it because Temporal does not read a database name from the secret at all: `DBNAME: temporal` and `VISIBILITY_DBNAME: temporal_visibility` are set literally on the server workload, and the `auto-setup` entrypoint creates both stores itself using the same superuser credentials. `credentials.database` only seeds the database the postgres subchart initializes (`POSTGRES_DB`). Consequence: changing `credentials.database` does NOT move Temporal's stores — it just creates an extra unused database. That was equally true in 1.0.x; no guard was added because an install that already did it is working, and a guard would break its upgrade.
