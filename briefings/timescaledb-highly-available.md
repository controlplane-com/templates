...
| volumeset, 1 config secret, 2 script secrets, identity, policy | Per-replica storage; credentials; Patroni/HAProxy start scripts; least-privilege secret reveal |
...
| `postgres.{username,password,database}` | username/password/test | template-managed superuser — **change the password** |
...
- **Changing `postgres.password` after first boot does nothing** — credentials are baked into the data directory at bootstrap. To reset, uninstall (deletes volumes) and reinstall.
...
- **TimescaleDB activation** relies on `shared_preload_libraries = timescaledb` in the Patroni config plus a `CREATE EXTENSION` post-init — if a user reports "no timescaledb functions," confirm the extension exists in *their* database (it's created in `postgres.database` only).
