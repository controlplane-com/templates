...
- **`allowMasterKeys` is a real behaviour change in 1.3.0.** Upstream Tyk defaults `allow_master_keys` to `false` (Go zero value; no default tag in `config/config.go`). Anyone whose workflow relied on `POST /tyk/keys` with a bare session object will start getting a refusal on 1.3.0 and must either add `access_rights` (correct) or set `allowMasterKeys: true` (deliberate). **The refusal is not self-explanatory, which is the trap** — the measured responses name neither master keys nor the setting: `POST /tyk/keys/create` returns `400 Failed to create key, keys must have at least one Access Rights record set.`, and `POST /tyk/keys/{custom-key}` returns `500 Failed to create key, ensure security settings are correct.` The canonical reason appears **only in the gateway log**, as `Master keys disallowed in configuration, key not added.` with `err="master keys disabled"`. Diagnose this from `cpln logs`, not from the API response.
