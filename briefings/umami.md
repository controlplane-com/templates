...
- **The secret name is org-wide, not release-scoped.** That is forced by the Helm limitation above — a subchart value cannot contain `.Release.Name`. Two umami releases in one org left at the default would both render `my-umami-db-credentials` — and **measured, the second install is REFUSED** (`cannot be updated because it is being managed by a different release`) and creates nothing. Nothing is shared, overwritten or deleted, and the first release is unaffected; you simply cannot install the second until you give it a distinct name. A hand-made secret cannot be adopted either (rejected for `missing the 'cpln/release' tag`). The values comment and README say to give each release its own name; there is no render-time guard for it.
