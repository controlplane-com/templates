...
- **A database that never becomes reachable crash-loops the app on purpose.** The app waits up to 240 s for PostgreSQL to answer the Postgres protocol, then exits with `[calcom] FATAL: PostgreSQL at ... did not answer`; a failed `prisma migrate deploy` exits too. That is deliberate — the alternative, which this template shipped before, is Cal.com serving 200s against an empty database with every health surface green. The wait restarts from the beginning on every restart, so a database that takes longer than 240 s is still waited out; you simply get the FATAL line each time round instead of an unexplained restart. Read `cpln logs '{gvc="{gvc}", workload="{release}-calcom"}' --limit 50` and fix the backing store; the app recovers by itself once the database serves.
