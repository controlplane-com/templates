...
A block that exposes both a floor and a limit names them `minCpu`/`maxCpu` and `minMemory`/`maxMemory`. A block that exposes only the limit uses the API's own field names, bare `cpu`/`memory`. **`worker.resources` and `redis.resources` still use `maxCpu`/`maxMemory`**, and renaming them is proposed separately since it would break existing values — they were `maxCpu`/`maxMemory` in 1.0.0, with no floor to contrast against.
...
2. Install this version as a **new release** into an existing single-location GVC, with a new `DATABASE_URL` in a new credentials secret pointing at the new release's Postgres. Both prerequisite secrets are not release-managed, so you can reuse the advisor credentials secret if you also point `DATABASE_URL` at the new host.
...
One values key also changed, a clean break with no fallback:

- `gvc.locations` (a list) became the top-level `location` (a single name).

## Important Notes

- **Never `helm upgrade` a release of the earlier GVC-creating 1.0.0 onto this one.** It deletes the GVC that version adopted and everything in it. See [If you installed the earlier 1.0.0](#if-you-installed-the-earlier-100).
