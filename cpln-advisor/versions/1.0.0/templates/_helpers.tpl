...
{{/*
GVC-read Policy Name — for the boot-time GVC reconciliation.
*/}}
...
the conversion the links were hand-written `//gvc/{{ .Values.global.cpln.gvc }}/workload/…`
...
invariant now that this chart does not own its GVC. Previously the chart
...
`cpuKey`/`memKey` name the keys in the caller's block, because blocks differ in
...
{{/*
The chart no longer creates a GVC. Refuse to render if the values still
...
Helm has owned it ever since. An in-place `helm upgrade` onto this chart drops
...
{{- fail "cpln-advisor: the `gvc` values key was REMOVED. This chart no longer creates a GVC — it deploys into the GVC you install into, and `gvc.locations` became the single top-level `location`. DO NOT `helm upgrade` a release of the earlier GVC-creating 1.0.0 onto this chart: it created a GVC NAMED AFTER THE ONE YOU INSTALLED INTO, so Helm ADOPTED your GVC and owns it. The upgrade drops `kind: gvc` from the manifest and Helm deletes what a chart no longer declares, which DESTROYS that GVC and every workload, volumeset and identity inside it — your advisor database included, and anything else you happen to keep there. Install this chart as a NEW release against an existing GVC, move the database across, then uninstall the old release. See `Migrating from 1.0.0` in the README." -}}
{{- end -}}
{{- end -}}
