...
- **WhatsApp (personal) is opt-in** (`whatsapp.enabled: true`) and pairs via a QR after install — the bridge's dependencies install to the volume on first pairing and the session is stored on the volume, retained across restarts. It links your own WhatsApp account; the default self-chat mode is owner-scoped, and bot mode should be given an allowlist during setup. The WhatsApp Cloud (business) API is not wired by this template.
