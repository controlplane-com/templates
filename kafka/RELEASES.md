...
- **Rack-aware fetching to cut cross-zone traffic (KIP-392)**: The Kafka cluster now supports rack awareness so consumers can read from a same-zone replica instead of always the (possibly cross-zone) partition leader, reducing cross-zone data-transfer cost in multi-zone deployments. Configured under the new `kafka.rackAwareness` section in `values.yaml`. **AWS-only**, and **disabled by default** (`enabled: false`) — it depends on the `AWS_ZONE_ID` env var, which is only present on AWS locations, so enabling it elsewhere just emits dead config.
  ...
  - **Non-AWS behavior**: when `AWS_ZONE_ID` is absent, `broker.rack` is left unset and the selector transparently falls back to leader-only fetches (topic creation never fails on partial rack info). If the feature is enabled anyway on such a location, the init script logs an `INFO` line noting `broker.rack` was left unset. Keep it disabled off AWS.
  - Most beneficial when brokers are actually spread across zones (see `kafka.multiZone`).

- **Custom tags for Kafka Connect workloads**: The `kafka_connectors` entry now accepts an optional `tags` map that is rendered on the connector workload resource. This lets users set platform-level tags such as `cpln/largeDisk: 'true'` without modifying the template.

- **Importable (externally-managed) log volume sets**: The Kafka cluster can now mount log volume sets that are managed **outside** the chart, via the new `kafka.volumes.logs.externalVolumeSets` list (one entry per `kafka.logDirs`, same order; empty string = keep chart-managed). By default the chart continues to create and manage volume sets named `<release-name>-logs-<index>` (e.g. `kafka-logs-0`).
