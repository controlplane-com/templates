...
- **Raising the image tag on a populated volume fails by design** — Meilisearch refuses to open a database written by an older version. Set `server.upgradeDb: true` for exactly one deploy, and **once the workload is ready again on the new tag, set it back to `false` and redeploy** so a migration flag is not left permanently on. The upgrade is **not atomic**: confirm a recent backup exists first.
...
- **Single replica, by design and by edition.** Replication and sharding are Meilisearch Enterprise features and are not compiled into the Community image this template ships, so there is no `replicas` knob and no failover. A rolling restart or upgrade takes the search endpoint down for about **87 seconds** (measured: 68 consecutive failed requests out of 349 at 1 req/s across a `helm upgrade`) — and the failures start **~51 s after the upgrade command returns**, so the CLI looks finished well before the outage begins. An abrupt replica loss recovers faster, in about **20 seconds**, with data intact. Have your application fall back to a database query when search is unavailable.
