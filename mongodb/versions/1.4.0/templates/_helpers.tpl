...
{{- define "mongo.validate" -}}
{{- if or (hasKey .Values.config "username") (hasKey .Values.config "password") -}}
{{- fail "mongodb: config.username and config.password were REMOVED — they are now a `dictionary` secret you create, named by config.credentialsSecretName, holding the keys `username`, `password` and `database`. Delete them from your values. See Prerequisites in the README." -}}
{{- end -}}
{{- if not .Values.config.credentialsSecretName -}}
{{- fail "mongodb: config.credentialsSecretName is required — it names the `dictionary` secret holding `username`, `password` and `database`. Create that secret BEFORE installing; see Prerequisites in the README." -}}
{{- end -}}
{{- end -}}
