...
MySQL is the world's most widely used open-source relational database. This template deploys a single MySQL instance on a persistent volume, with an optional phpMyAdmin console and optional scheduled dumps to S3 or GCS.

## Architecture

- **Workload** (`stateful`) — MySQL serving TCP on port 3306, one replica.
...
- **Backup workload** (`cron`, optional) — scheduled `mysqldump` to an S3 or GCS bucket via a Control Plane cloud account.
...
**Application credentials** (`credentialsSecretName`) — a `dictionary` secret with exactly the keys `username`, `password` and `database`. MySQL creates this user and this database on first boot, and this is the credential you put in your applications' connection strings:
...
**Root password** (`rootPasswordSecretName`) — an `opaque` secret (`encoding: plain`) holding the MySQL `root` password. It is kept separate from the credentials above so that sharing the application credentials with a teammate or another workload does not hand out root:
...
```yaml
credentialsSecretName: my-mysql-credentials   # dictionary secret: username, password, database — must EXIST BEFORE INSTALL
rootPasswordSecretName: my-mysql-root-password # opaque secret (encoding: plain): the root password — must EXIST BEFORE INSTALL
```
...
MySQL is never published to the internet by this template. Reach it from inside the GVC, or from outside through your own proxy.
...
<b>Upgrading from 1.5.0:</b> this version removes <code>aws::ReadOnlyAccess</code> from the backup identity.
...
- **Credentials are read only when the volume is first initialized.** Rotating either secret afterwards does not change the stored passwords; change them with `ALTER USER` inside MySQL instead.
- **Do not scale past one replica.** This is a single-instance MySQL on one volume; extra replicas would mount nothing and corrupt nothing usefully.
...
- **Upgrading from 1.4.x is a breaking change.** `config.*` and `enablePhpMyAdmin` no longer exist — an install that still sets them fails at render with instructions. Create the two secrets first, and expect the credentials on an existing volume to stay whatever they already were.
