...
- **Resource names no longer carry the `-ml` infix, and that is a FRESH-INSTALL-ONLY change.** Every PostgreSQL-side workload, secret, identity, policy and — critically — the **volume set** is now named `{release}-postgres…` instead of `{release}-postgres-ml…`. The bundled etcd resources were NOT renamed (`{release}-etcd`, `{release}-etcd-vs` and the rest are unchanged), so etcd's volume set is not orphaned by this. Running `helm upgrade` over an install created before this change creates a **new, empty volume set** and leaves the old one behind holding your data (and still billing). There is no in-place upgrade path: back up (`backup.mode: logical` or a `pg_dumpall`), uninstall, reinstall, restore, then delete the orphaned volume set.
