...
Set your desired backup schedule in the values file and configure your AWS S3, GCS, or MinIO bucket. You can also set a prefix where your backups will be stored in the bucket.
...
Backs up to a self-hosted MinIO workload (or any other S3-compatible endpoint) — no Control Plane Cloud Account required, since credentials are passed directly to the backup job.

1. Create your bucket in MinIO. Update `bucket` to include its name.

2. Set `endpoint` to the MinIO S3 API address. For the `minio` marketplace template deployed in the same GVC, this is `http://WORKLOAD_NAME:9000`.
