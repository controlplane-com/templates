...
- **`ready` means the node answers, not that the cluster exists.** The readiness probe asks the local server for a `PONG`; it cannot also require `cluster_state:ok`, because the cluster is not created until every node is up — a probe that waited for it would deadlock the ordered rollout. So during first boot a node can report ready with no cluster yet, for up to about a minute while creation retries. Check `CLUSTER INFO`, not the workload's ready flag, to know the cluster is formed.
