...
- **History Server needs an existing bucket** — enable `historyServer.enabled` AND set `storage.*` to a **real** bucket + cloud account. If the bucket does not exist or is unreachable the History Server **crash-loops** (`UnknownStoreException`/`NoSuchBucket` in its log). An empty History Server list is normal until a job completes. The S3A connector jars are downloaded from Maven Central at container start on every driver tier and the History Server.
